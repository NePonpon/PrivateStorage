#> private_storage:sys/provide/1/
#
# シフト1回目
#

execute store result storage private_storage:sys provide.target int 1 run data get storage private_storage:sys provide_id 8192
function private_storage:sys/provide/get_octal
data modify storage private_storage:macro provide.shift set from storage private_storage:sys provide.target

execute unless data storage private_storage: _[0] run data modify storage private_storage: _ set from storage private_storage:sys initial

data remove storage private_storage: _[14]
data remove storage private_storage: _[13]
data remove storage private_storage: _[12]
data remove storage private_storage: _[11]
data remove storage private_storage: _[10]
data remove storage private_storage: _[9]
data remove storage private_storage: _[8]

function private_storage:sys/provide/1/shift with storage private_storage:macro provide

function private_storage:sys/provide/2/
